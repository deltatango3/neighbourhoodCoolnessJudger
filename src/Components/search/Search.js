...
class Search extends Component {
  ...
  // 	})
  // }
  async returnConcerts(lat, lng) {
    const concerts = [];
    const data = await getConcerts(lat, lng);

    if (data.data._embedded.events) {
      data.data._embedded.events.forEach(concert => {
        concerts.push(concert);
      });

      for (let i = 1; i < data.data.page.totalPages; i++) {
        const data = await getConcerts(lat, lng, i);
        data.data._embedded.events.forEach(concert => {
          concerts.push(concert);
        });
      }
    ...
    return (
      <div className="searchContainer wrapper">
        ...
        <div className="searchInfo">
          ...
          <p>
            Enter your address below to find out whether your neighbourhood is a
            hot block – or a nerdy dungeon.
          </p>
        </div>

        <form className="search" onSubmit={this.handleSubmit}>
          <label htmlFor="searchbar" className="visuallyHidden">
            Search
          </label>
          <div className="searchBoxContainer">
            <input
              ...
              required
            />
            <label htmlFor="location" className="floatingLabel">
